...
var tokenCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		tokenExpiry := viper.GetInt("tokenExpiry")
		...
		if roleARN != "" {
			...
			tok, err = gen.GetWithRole(clusterID, roleARN, tokenExpiry)
		} else {
			// otherwise sign the token with immediately available credentials
			tok, err = gen.Get(clusterID, tokenExpiry)
		}
		if err != nil {
			fmt.Fprintf(os.Stderr, "could not get token: %v\n", err)
			os.Exit(1)
...
func init() {
	...
	tokenCmd.Flags().Int("token-expiry", 900, "Expiry duration for Bearer token in seconds. Default is 15 minutes")
	viper.BindPFlag("role", tokenCmd.Flags().Lookup("role"))
	viper.BindPFlag("tokenOnly", tokenCmd.Flags().Lookup("token-only"))
	viper.BindPFlag("tokenExpiry", tokenCmd.Flags().Lookup("token-expiry"))
